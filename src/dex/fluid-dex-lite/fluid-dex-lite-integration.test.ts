...
/*
  ...
  This test script adds tests for FluidDexLite general integration
  with the DEX interface. The test cases below are example tests.
  It is recommended to add tests which cover FluidDexLite specific
  logic.
...
async function checkOnChainPricing(
  ...
  // Normally you can get it from fluidDexLite.Iface or from eventPool.
  ...
  const readerResult = (
    await fluidDexLite.dexHelper.multiContract.methods
...
describe('FluidDexLite', function () {
  ...
  describe('Mainnet', () => {
    ...
    it('getTopPoolsForToken', async function () {
      ...
      const newFluidDexLite = new FluidDexLite(network, dexKey, dexHelper);
      if (newFluidDexLite.updatePoolState) {
        await newFluidDexLite.updatePoolState();
      }
      const poolLiquidity = await newFluidDexLite.getTopPoolsForToken(
        tokens[srcTokenSymbol].address,
        10,
      );
      console.log(`${srcTokenSymbol} Top Pools:`, poolLiquidity);

      if (!newFluidDexLite.hasConstantPriceLargeAmounts) {
        checkPoolsLiquidity(
          poolLiquidity,
