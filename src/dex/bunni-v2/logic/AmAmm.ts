...
import { Bid, DexParams, PoolState } from '../types';
...
export function getTopBid(
  ...
  const hook = config.bunniHooks.find(
    bunniHook =>
      bunniHook.address.toLowerCase() === state.key.hooks.toLowerCase(),
  );
...
export function getTopBidWrite(
  ...
  if (hook) {
    _updateAmAmmWrite(state, blockNumber, hook.deploymentBlock, config.K);
  }
...
export function _updateAmAmmWrite(
  ...
  // TODO
  // solidity checks for last updated block here
  // do we need to do the same or is it ok to run this
